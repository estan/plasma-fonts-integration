...
void KdePlatformTheme::showPlatformMenuBar()
{
...
QPixmap KdePlatformTheme::standardPixmap(QPlatformTheme::StandardPixmap sp, const QSizeF &size) const
{
...
QPixmap KdePlatformTheme::fileIconPixmap(const QFileInfo &fileInfo, const QSizeF &size, QPlatformTheme::IconOptions iconOptions) const
{
    ...
    return KIconLoader::global()->loadMimeTypeIcon(
        QMimeDatabase().mimeTypeForFile(fileInfo).iconName(), KIconLoader::Desktop, size.toSize().width());
}
